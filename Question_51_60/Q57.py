...
def Template_matching(img, template):
    ...
    _template = template - np.mean(template,axis=(0,1))
    ...
    for y in range(0,H-ht):
        for x in range(0,W-wt):
            _s = np.sum(_img[y:y+ht,x:x+wt] * _template)
            _s /= np.sqrt(np.sum(_img[y:y+ht,x:x+wt]**2))*np.sqrt(np.sum(_template**2))
            if _s > S:
                S = _s
                i, j = x, y
...
# Save result
cv2.imwrite("answer_57.jpg", out)
cv2.imshow("answer_57.jpg", out)
cv2.waitKey(0)
